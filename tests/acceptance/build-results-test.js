...
module('Acceptance | build results', function (hooks) {
  ...
  test('sorts results by run date', async function (assert) {
    ...
    assert
      .dom('.test-build-result')
      .hasAttribute('data-testResultId', `${latestTestResult.id}`);

    let results = findAll('.test-build-result');
    assert
      .dom(results[1])
      .hasAttribute('data-testResultId', `${middleTestResult.id}`);
    assert
      .dom(results[2])
      .hasAttribute('data-testResultId', `${earliestTestResult.id}`);
  });
  ...
  test('displays appropriate status based on result', async function (assert) {
    ...
    assert
      ...
      .containsText(
        'failed - timed out',
        'displays failure notice with status message for failed builds'
      );
    assert
      .dom(results[1])
  ...
  test('displays semver string for non-canary builds', async function (assert) {
    ...
    assert
      .dom('.test-build-result')
      .containsText('>= 2.0.0', 'displays semver string');

    await click(findByText('.test-build-result a', 'details'));

    assert
      .dom('.test-semver-string')
      .hasText('>= 2.0.0', 'displays semver string');
  });
  ...
  test('displays appropriate indication for canary builds', async function (assert) {
    ...
    assert
      .dom('.test-build-result')
      .containsText('canary', 'displays indication for canary builds on list');

    await click(findByText('.test-build-result a', 'details'));
    assert
      .dom('.test-semver-string')
      .hasText('canary', 'displays indication for canary builds in detail');
  });
  ...
  test('links to previous day', async function (assert) {
    ...
    assert
      .dom('a[href="/admin/build-results?date=2017-01-31"]')
  ...
  test('links to following day if not viewing the current date', async function (assert) {
    ...
    assert
      .dom('a[href="/admin/build-results?date=2016-11-19"]')
  ...
  test('does not link to following day if viewing the current date', async function (assert) {
    ...
    assert
      .dom(`a[href="/admin/build-results?date=${tomorrow}"]`)
      .doesNotExist(
  ...
  test('detail page shows data for a build', async function (assert) {
    ...
    assert
      .dom('.test-output')
      .hasText('this is the output', "displays result's output");
  });
  ...
  test('detail page has a "retry" button for failed builds', async function (assert) {
    ...
    assert
      ...
      .doesNotExist('"retry" button removed after retrying');
  });
