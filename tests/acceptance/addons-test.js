...
module('Acceptance: Addons', function (hooks) {
  ...
  test('displays 0 for score when addon has zero score', async function (assert) {
    ...
    assert
      ...
      .hasText(
        'Score 0.0',
        'Displays 0 for score when addon has a score of zero'
      );
  });
  ...
  test('displays ? for score when addon has no score', async function (assert) {
    ...
    assert
      .dom('.score')
      .containsText('?', 'Displays ? for score when addon has no score');
  });
  ...
  test('displays categories', async function (assert) {
    ...
    assert
      .dom('.test-category-list')
      .containsText('Another category for categories');
  });
  ...
  test('displays n/a warning for score info on addon that has not been reviewed', async function (assert) {
    ...
    assert
      .dom('.test-not-reviewed-warning')
      .containsText('N/A - This addon has not yet been reviewed.');
  });
  ...
  test('displays score calculation', async function (assert) {
    ...
    assert
      .dom(checks[0])
      .containsText('The addon only uses sustainably-sourced coffee');
    assert.dom('.test-value', checks[0]).containsText('0.7');
    assert
      .dom('.test-value', checks[0])
      .hasClass(
        'yellow',
        'Has class to indicate mediocre check passing percentage'
      );
    assert
      .dom('.test-max-contribution', checks[0])
      .containsText(
        '3.33',
        'Displays max contribution in the scale of the score'
      );

    assert.dom(checks[1]).containsText('Mines bitcoin');
    assert.dom('.test-value', checks[1]).containsText('0.3');
    assert
      .dom('.test-value', checks[1])
      .hasClass('red', 'Has class to indicate poor check passing percentage');
    assert
      .dom('.test-max-contribution', checks[1])
      .containsText(
        '3.33',
        'Displays max contribution in the scale of the score'
      );
    ...
    assert
      .dom('.test-value', checks[2])
      .hasClass(
        'green',
        'Has class to indicate perfect check passing percentage'
      );
    assert
      .dom('.test-max-contribution', checks[2])
      .containsText(
        '3.33',
        'Displays max contribution in the scale of the score'
      );
  ...
  test('displays header', async function (assert) {
    ...
    assert
      .dom('.test-addon-header')
      .containsText('test-addon', 'Header displays addon name');
    assert
      .dom('.test-addon-flag-deprecation')
      .doesNotExist('Deprecation flag does not display');
    assert
      .dom('.test-addon-flag-new')
      .doesNotExist('New flag does not display');
    assert
      .dom('.test-addon-flag-official')
      .doesNotExist('Official flag does not display');
    assert
      .dom('.test-addon-flag-cli-dependency')
      .doesNotExist('CLI Dependency flag does not display');
  ...
  test('displays addon stats', async function (assert) {
    ...
    assert
      .dom('.test-addon-install-command')
      .hasText('ember install test-addon');
    ...
    assert
      .dom('.test-addon-top-downloaded')
      .containsText('1,564 downloads in last month');

    assert
      .dom(
        '.test-addon-demo-url a[href="http://www.example.com/demo_of_addon"]'
      )
      .exists();
    assert
      .dom('.test-addon-demo-url')
      .containsText('example.com/demo_of_addon');

    assert
      .dom('.test-addon-repo-url a[href="http://www.example.com/addon_repo"]')
      .exists();
    assert.dom('.test-addon-repo-url').containsText('example.com/addon_repo');

    assert
      .dom(
        '.test-addon-package-url a[href="https://www.npmjs.com/package/test-addon"]'
      )
      .exists();
    assert
      .dom('.test-addon-package-url')
      .containsText('npmjs.com/package/test-addon');

    assert
      .dom('.test-addon-license a[href="https://spdx.org/licenses/MIT"]')
      .hasText('MIT');
    assert
      .dom('.test-addon-keywords')
      .hasText('keyword-0, keyword-1, keyword-2, keyword-3, keyword-4');

    assert
      .dom('.test-addon-maintainers a')
      .exists({ count: 3 }, 'maintainers should display');
    assert
      .dom('.test-addon-maintainers a[href*="/maintainers/maintainer-0"]')
      .exists('maintainers should have link to them');
    assert
      .dom(
        '.test-addon-maintainers img[src="https://secure.gravatar.com/avatar/412412d3d6d6fc8809f9121216dd0?d=identicon"]'
      )
      .exists('maintainers should have gravatar');
    assert
      .dom(
        '.test-addon-maintainers img[title="maintainer-0"][alt="maintainer-0"]'
      )
      .exists('Title and source of gravatar are set');

    assert.dom('.test-addon-ember-cli-version').hasText('1.13.1');

    assert.dom('.test-addon-version-count').hasText('versions (2)');
    assert
      .dom('.test-addon-versions li')
      .exists(
        { count: 3 },
        'Only 3 list items should be under versions, 2 for versions, 1 for ember versions after publishing of addon'
      );
    ...
    assert
      .dom('.test-addon-badge .test-show-badge-markdown .icon-content-paste')
      .exists('Show badge markdown to copy');

    await click('.test-addon-badge .test-show-badge-markdown');
    assert
      .dom('.test-addon-badge .test-badge-markdown')
      .hasText(
        '[![Ember Observer Score](https://emberobserver.com/badges/test-addon.svg)](https://emberobserver.com/addons/test-addon)'
      );
  });
  ...
  test('addon has an invalid github repo', async function (assert) {
    ...
    assert
      .dom('.test-addon-flag-invalid-repo')
      .exists('displays invalid repo message');
    assert
      .dom('.test-addon-repo-url a')
      .doesNotExist('does not display repo url');
  ...
  test('displays addon dependencies', async (assert) => {
    ...
    assert
      .dom('.test-addon-dependencies .test-dependencies .test-dependency-name')
      .hasText(dependency.package);
    assert
      .dom('.test-addon-dependencies .test-dependency-name')
  ...
  test('displays addon dependency size info when available', async (assert) => {
    ...
    assert
      .dom(
        `.test-dependency-${dependencyWithSize.package} .test-dependency-name`
      )
      .containsText(
        dependencyWithSize.package,
        'Package name still shows along with size'
      );
    assert
      .dom(`.test-dependency-${dependencyWithSize.package} .test-size-summary`)
      .containsText('1.61 KB (1.05 KB gzipped)', 'Total size shows');

    assert
      .dom(
        `.test-dependency-${dependencyWithNoSize.package} .test-size-summary`
      )
      .doesNotExist('No size summary shows if none found');
    assert
      .dom(`.test-dev-dependency-${devDependency.package} .test-size-summary`)
      .doesNotExist('No size data shown for dev dependencies');

    await click(
      `.test-dependency-${dependencyWithSize.package} .test-size-summary`
    );
    assert
      .dom(`.test-dependency-${dependencyWithSize.package} .test-size-detail`)
      .exists('Size detail shows');

    await click(
      `.test-dependency-${dependencyWithSize.package} .test-other-assets-toggle`
    );
    assert
      .dom(
        `.test-dependency-${dependencyWithSize.package} .test-other-assets-details`
      )
      .exists('Other assets detail shows');
  ...
  test('hides some dependencies if there are over a certain amount', async function (assert) {
    ...
    let dependencyPackages = findAll(
    ...
    let devDependencyPackages = findAll(
  ...
  test('when there are no addon dependencies', async (assert) => {
    ...
    assert
      .dom('.test-addon-dependencies .test-dev-dependencies')
      .doesNotExist();
  });
  ...
  test('displays addon dependents', async (assert) => {
    ...
    assert
      .dom('.test-addon-dependents .test-dependencies')
      .doesNotExist('Not showing dependents');
    assert
      .dom('.test-addon-dependents .test-dev-dependencies')
      .doesNotExist('Not showing dependents');

    await click('.test-show-dependents');

    let dependencyPackages = findAll(
      '.test-addon-dependents .test-dependencies .test-dependency-name'
    );
    assert.equal(
      dependencyPackages.length,
      7,
      'Shows list of dependent addons'
    );

    let devDependencyPackages = findAll(
      '.test-addon-dependents .test-dev-dependencies .test-dependency-name'
    );
    assert.equal(
      devDependencyPackages.length,
      8,
      'Shows list of dev dependent addons'
    );
  });
  ...
  test('when addon dependencies fail to load', async (assert) => {
    ...
    assert
      .dom('.test-addon-dependents .test-uncollapse-error')
      .exists('Shows error message');
    resetOnerror();
  });
  ...
  test('has a link for users to provide suggestions', async function (assert) {
    ...
    assert.equal(
      currentURL(),
      '/addons/test-addon/correct',
      'suggest a correction link works'
    );
  });
  ...
  module('Scoped addons', function (hooks) {
    ...
    test('displays badge for scoped addon', async function (assert) {
      ...
      assert
        .dom('.test-addon-badge img[src="/badges/-foo-bar-test-addon.svg"]')
        .exists();
      ...
      assert
        .dom('.test-addon-badge .test-badge-markdown')
        .hasText(
          '[![Ember Observer Score](https://emberobserver.com/badges/-foo-bar-test-addon.svg)](https://emberobserver.com/addons/@foo-bar/test-addon)'
        );
    });
  });
});
