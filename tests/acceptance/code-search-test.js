...
module('Acceptance | code search', function (hooks) {
  ...
  test('searching for addons containing code', async function (assert) {
    ...
    assert
      .dom('.test-addon-name')
      .exists({ count: 3 }, 'All addons in results show');

    assert
      .dom(`[data-id="${firstAddon.id}"] .test-addon-name`)
      .containsText('ember-try', 'Addon name shows');
    assert
      .dom(`[data-id="${firstAddon.id}"] .test-addon-name`)
      .hasAttribute(
        'href',
        '/addons/ember-try',
        'Addon name links to addon page'
      );
    assert
      .dom(`[data-id="${firstAddon.id}"] .test-usage-count`)
      .hasText('1 usage', 'Addon usage count shows');
    assert
      .dom('.test-result-info')
      .containsText('6 usages', 'Total usage count shows');
  ...
  test('viewing addon source containing search query', async function (assert) {
        ...
        results: [
          {
            ...
            lines: [
              ...
              {
                text: 'addons.pushObject({ addon: addonData.addon });',
                number: 52,
              },
              { text: '}', number: 53 },
            ],
    ...
    assert
      ...
      .hasText(
        'Results for "asdf" — Found 1 addon (2 usages)',
        'Last search shows'
      );
    ...
    assert
      .dom('.test-usage')
      .containsText(
        'app/services/fake-service.js:52',
        'Filename shows with line number of match'
      );
    assert
      .dom('.test-usage')
      .containsText(
        '51if (addonData) {',
        'Source code lines show with line numbers'
      );
    ...
    assert
      .dom(matches[0])
      .hasText(
        'addons.pushObject({ addon: addonData.addon });',
        'First match is highlighted correctly'
      );
    assert
      .dom(matches[1])
      .hasText(
        'store: inject.service(),',
        'Second match is highlighted correctly'
      );

    let secondUsage = findAll('.test-usage')[1];
    assert
      .dom(secondUsage)
      .containsText('app/components/fake-thing.js:21', 'Shows both usages');

    assert
      .dom(findByText('.test-usage', 'fake-service').querySelector('a'))
      .hasAttribute(
        'href',
  ...
  test('sorting search results', async function (assert) {
    ...
    assert
      .dom(addonNames[0])
      .containsText('ember-blanket', 'Default sort is ascending by addon name');
    assert
      .dom(addonNames[1])
      .containsText('ember-foo', 'Default sort is ascending by addon name');
    assert
      .dom(addonNames[2])
      .containsText('ember-try', 'Default sort is ascending by addon name');
    ...
    assert
      .dom(resortedAddonNames[0])
      .containsText(
        'ember-foo',
        'Addons are sorted descending by default for switch to usage count sort'
      );
    assert
      .dom(resortedAddonNames[1])
      .containsText(
        'ember-blanket',
        'Addons are sorted descending by usage count'
      );
    assert
      .dom(resortedAddonNames[2])
      .containsText('ember-try', 'Addons are sorted descending by usage count');
    assert.equal(
      currentURL(),
      '/code-search?codeQuery=foo&sort=usages&sortAscending=false',
      'Sort and sort direction is in query params'
    );
    ...
    assert
      ...
      .containsText(
        'ember-try',
        'Addons are sorted ascending by usage count when direction toggled'
      );
    assert
      .dom(resortedAddonNames[1])
      .containsText(
        'ember-blanket',
        'Addons are sorted ascending by usage count'
      );
    assert
      .dom(resortedAddonNames[2])
      .containsText('ember-foo', 'Addons are sorted ascending by usage count');
    assert.equal(
      currentURL(),
      '/code-search?codeQuery=foo&sort=usages',
      'Sort is in query params'
    );
    assert.dom(usageSortButton).hasClass('selected');
    ...
    assert
      .dom(resortedAddonNames[0])
      .containsText('ember-blanket', 'Addons sorted ascending addon name');
    assert
      .dom(resortedAddonNames[1])
      .containsText('ember-foo', 'Addons sorted ascending addon name');
    assert
      .dom(resortedAddonNames[2])
      .containsText('ember-try', 'Addons sorted ascending addon name');
    assert.equal(
      currentURL(),
      '/code-search?codeQuery=foo',
      'Sort and sort direction not in query params when default'
    );
    assert.dom(nameSortButton).hasClass('selected');
    assert.ok(nameSortButton.querySelector('.icon-expand-less'));
    ...
    assert
      .dom(resortedAddonNames[0])
      .containsText('ember-try', 'Addons sorted descending addon name');
    assert
      .dom(resortedAddonNames[1])
      .containsText('ember-foo', 'Addons sorted descending addon name');
    assert
      .dom(resortedAddonNames[2])
      .containsText('ember-blanket', 'Addons sorted descending addon name');
    assert.equal(
      currentURL(),
      '/code-search?codeQuery=foo&sortAscending=false',
      'Sort direction is in query params'
    );
    assert.dom(nameSortButton).hasClass('selected');
    ...
    assert
      .dom(scoreSortedAddonNames[0])
      .containsText(
        'ember-try',
        'Addons are sorted by descending score by default'
      );
    assert
      .dom(scoreSortedAddonNames[1])
      .containsText('ember-blanket', 'Addons are sorted by score');
    assert
      .dom(scoreSortedAddonNames[2])
      .containsText('ember-foo', 'Addons are sorted by score');
    assert.equal(
      currentURL(),
      '/code-search?codeQuery=foo&sort=score&sortAscending=false',
      'Sort is in query params'
    );
    ...
    assert
      .dom(latestVersionDateSortedNames[0])
      .containsText(
        'ember-foo',
        'Addons are sorted by descending latest version date by default'
      );
    assert
      .dom(latestVersionDateSortedNames[1])
      .containsText('ember-try', 'Addons are sorted by latest version date');
    assert
      .dom(latestVersionDateSortedNames[2])
      .containsText(
        'ember-blanket',
        'Addons are sorted by latest version date'
      );
    assert.equal(
      currentURL(),
      '/code-search?codeQuery=foo&sort=updated&sortAscending=false',
      'Sort is in query params'
    );
  });
  ...
  test('searching with a regex', async function (assert) {
    ...
    assert
      .dom('.test-regex-search:checked')
      .exists('Regex checkbox is checked');
    assert
      .dom('.test-last-search')
      .hasText(
        'Results for /foo/ — Found 1 addon (2 usages)',
        'Last search shows as a regex'
      );
    assert
      .dom('.test-regex-help')
      .hasAttribute(
    ...
    assert
      ...
      .hasText(
        'Results for /foo/ — Found 1 addon (2 usages)',
        'Last search still shows as a regex'
      );
    ...
    assert
      .dom('.test-regex-help')
      .doesNotExist('Regex syntax link does not show when regex is deselected');
  ...
  test('searching when sort is set in query param', async function (assert) {
    ...
    assert
      .dom(addonNames[0])
      .containsText('ember-foo', 'Addons are sorted by usages');
    assert
      .dom(addonNames[1])
      .containsText('ember-blanket', 'Addons are sorted by usages');
    assert
      .dom(addonNames[2])
      .containsText('ember-try', 'Addons are sorted by usages');
  });
  ...
  test('filtering search results by file path', async function (assert) {
    ...
    assert
      .dom('.test-addon-name')
      .exists({ count: 3 }, 'shows all addons before filtering');

    await fillIn('.test-file-filter-input', filterTerm);

    assert
      .dom('.test-addon-name')
      .exists({ count: 2 }, 'shows only matching addons after filtering');
    assert
      .dom('.test-result-info')
      .containsText(
        '3 addons',
        'full result count shows when filter is applied'
      );
    assert
      .dom('.test-result-info')
      .containsText(
        '6 usages',
        'full usage count shows when filter is applied'
      );
    assert
      .dom('.test-filtered-result-info')
      .containsText(
        '2 addons',
        'filtered result count shows when filter is applied'
      );
    assert
      .dom('.test-filtered-result-info')
      .containsText(
        '3 usages',
        'filtered usage count shows when filter is applied'
      );
    ...
    assert
      .dom('.test-addon-name')
      .exists({ count: 3 }, 'shows all addons after clearing filter');
    ...
    assert
      .dom('.test-addon-name')
      .exists({ count: 1 }, 'shows only matching addons after filtering');
    assert
      .dom('.test-filtered-result-info')
      .containsText(
        '1 addon',
        'filtered result count is correct after regex search'
      );
    assert
      .dom('.test-filtered-result-info')
      .containsText(
        '1 usage',
        'filtered usage count is correct after regex search'
      );
  });
  ...
  test('filtering addon source by file path', async function (assert) {
        ...
        results: [
          {
            ...
            count: 2,
            files: [
              'app/components/no-match.js',
              'app/templates/components/no-match.hbs',
            ],
          },
          {
        ...
        results: [
          {
            ...
            lines: [{ text: 'if (addonData) {', number: 51 }],
          },
          {
            line_number: 21,
            filename: 'app/services/no-match.js',
            lines: [{ number: 20, text: '' }],
          },
        ],
        /* eslint-disable camelcase */
      };
    ...
    assert.equal(
      findByText('.test-usage', 'app/services/no-match.js'),
      null,
      'filtered out file does not show'
    );
    assert
      .dom('.test-usage')
      .containsText(
        'app/controllers/index.js',
        'file with matching name shows'
      );
    ...
    assert
      .dom(secondUsage)
      .containsText(
        'app/services/no-match.js',
        'previously filtered out file now shows'
      );
  });
  ...
  test('filtering works with sorting and pagination', async function (assert) {
    ...
    assert
      .dom('.test-addon-name')
      .exists({ count: 3 }, 'shows one page worth of addons after filtering');
    assert
      .dom('.test-filtered-result-info')
      .containsText(
        '4 addons',
        'filtered addon count shows when filter is applied'
      );
    assert
      .dom('.test-filtered-result-info')
      .containsText(
    ...
    assert
      .dom(firstAddonName)
      .containsText('ember-cli-thing', 'addons are sorted by usage count');

    await click('.test-view-more');

    assert
      .dom('.test-addon-name')
      .exists({ count: 4 }, 'adds additional addons that meet filter criteria');

    await click(findByText('.test-sort button', 'Name'));

    assert
      .dom('.test-filtered-result-info')
      .containsText('4 addons', 'filtered addon count shows after sorting');
    assert
      .dom('.test-filtered-result-info')
      .containsText('6 usages', 'filtered usage count shows after sorting');

    let firstAddonNameAfterSorting = findAll('.test-addon-name')[0];
    assert
      .dom(firstAddonNameAfterSorting)
      .containsText('ember-cli-matches', 'addons are sorted by name');

    await click('.test-clear-file-filter');

    assert
      .dom('.test-addon-name')
      .exists({ count: 3 }, 'shows first page of addons after clearing filter');
    assert
      .dom('.test-filtered-result-info')
      .doesNotExist('filtered result counts are gone after clearing');
    assert
      .dom('.test-result-info')
      .exists('total result counts are still showing');
    let firstAddonNameAfterClearingFilter = findAll('.test-addon-name')[0];
    assert
      .dom(firstAddonNameAfterClearingFilter)
      .containsText('ember-blanket', 'addons are sorted by name');
  });
  ...
  test('when file filter regex is invalid', async function (assert) {
    ...
    assert
      .dom('.test-addon-name')
      .doesNotExist('no addons show after filtering');
    assert
      .dom('.test-result-info')
      .containsText('3 addons', 'full result count still shows');
    assert
      .dom('.test-result-info')
      .containsText('6 usages', 'full usage count still shows');
    assert
      .dom('.test-filtered-result-info')
      .containsText('0 addons', 'filtered result count is 0');
    assert
      .dom('.test-filtered-result-info')
      .containsText('0 usages', 'filtered usage count is 0');
  });
  ...
  test('fixes links to code when repo URL ends with .git', async function (assert) {
    ...
    assert
      ...
      .hasAttribute(
        'href',
        'https://github.com/ember-cli/ember-try/tree/master/app/services/fake-service.js#L52'
      );
  });
