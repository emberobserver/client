...
module('Acceptance | Admin | Build servers', function (hooks) {
  ...
  test('not-logged-in users cannot see list of build servers', async function (assert) {
    ...
    assert.equal(
      currentURL(),
      '/',
      'not-logged-in users are redirected back to index page'
    );
  });
  ...
  test('adding a new build server', async function (assert) {
    ...
    assert
      .dom('[data-test-build-server]')
      .hasText(
        'new-server-name',
        'displays a row for the newly-created build server'
      );
    assert
      .dom('[data-test-new-build-server-name]')
      .hasValue('', 'server name input is cleared after save');
  });
});
