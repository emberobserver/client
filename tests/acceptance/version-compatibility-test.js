...
module('Acceptance | version compatibility', function (hooks) {
  ...
  test('displays Ember version compatibility when an addon has it', async function (assert) {
    let { addon } = createAddonWithVersionCompatibilities([
      failedVersion('1.13.13'),
      '2.0.0',
    ]);
    ...
    assert
      .dom('.test-ember-version-compatibility-list')
      .exists('version compatibility list displays');
    assert
      .dom('.test-ember-version-compatibility-ember-version')
      .hasText('2.0.0');
  ...
  test('sorts version compatibility entries by version number', async function (assert) {
    let { addon } = createAddonWithVersionCompatibilities([
      ...
      '2.10.0',
      '2.2.2',
      failedVersion('1.12.1'),
    ]);

    await visitAddon(addon);
    let versionCompatibilities = findAll(
      '.test-ember-version-compatibility-ember-version'
    );
  ...
  test("displays appropriate text when an addon's test result indicated a failure", async function (assert) {
    ...
    assert
      .dom('.test-ember-version-compatibility-section')
      .exists('displays the version compatibility section');
    assert
      .dom('.test-ember-version-compatibility-unknown')
      .exists('displays a message');
    assert
      .dom('.test-ember-version-compatibility-new-version-warning')
      .doesNotExist('"New version" warning does not display');
  });
  ...
  test('hides beta and canary versions from the table', async function (assert) {
    let { addon } = createAddonWithVersionCompatibilities([
      failedVersion('2.3.0'),
      '2.4.3',
      '2.5.0-beta.3+7c4288b9',
      '2.6.0-canary+e35e8b48',
    ]);
    ...
    assert
      ...
      .exists({ count: 2 });
    assert.notOk(
      findByText('.test-ember-version-compatibility-ember-version', 'beta'),
      'does not display beta versions'
    );
    assert.notOk(
      findByText('.test-ember-version-compatibility-ember-version', 'canary'),
      'does not display canary versions'
    );
  });
  ...
  test('displays semver string with compatibility when all tests passed', async function (assert) {
    ...
    await visitAddon(addon);
    assert
      .dom('.test-ember-version-compatibility-semver-compat')
      .containsText('Ember >=2.1.0 <=2.4.0');
  });

  test('displays date/time when tests were last run', async function (assert) {
    let { addon, testResult } = createAddonWithVersionCompatibilities([
      '2.1.0',
      '2.2.0',
      '2.3.0',
      '2.4.0',
    ]);
    server.db.testResults.update(testResult.id, {
      createdAt: moment.utc().subtract(1, 'day'),
    });

    await visitAddon(addon);
    assert
      .dom('.test-ember-version-compatibility-timestamp')
      .containsText('a day ago');
  });
  ...
  test('displays tests results from the latest version with them, if the newest version has none', async function (assert) {
    ...
    assert
      .dom('.test-ember-version-compatibility-new-version-warning')
      .exists('"New version" warning displays');
  });
  ...
  test('compatibility table is hidden but toggleable when all tests pass', async function (assert) {
    ...
    assert
      .dom('.test-ember-version-compatibility-list')
      .exists('version compatiblity list is displayed after clicking toggle');

    await click('.test-ember-version-compatibility-show-table');
    assert
      .dom('.test-ember-version-compatibility-list')
      .doesNotExist(
        'version compatibility list is not displayed after clicking toggle again'
      );
  });

  test('preface text for timestamp depends on status of tests', async function (assert) {
    let { addon: addonWithAllPassing } = createAddonWithVersionCompatibilities([
      '2.3.0',
      '2.4.0',
    ]);
    let {
  ...
  test('sets correct CSS class based on result', async function (assert) {
    ...
    assert.ok(
      results[0].querySelector('.result-passed'),
      'passing tests get the "result-passed" CSS class'
    );
    assert.ok(
      results[1].querySelector('.result-failed'),
      'failing tests get the "result-failed" CSS class'
    );
  });
  ...
  test('excludes canary-only builds for version compatiblity purposes', async function (assert) {
    ...
    testResults.concat(
      server.createList('testResult', 5, {
        canary: true,
        succeeded: true,
        createdAt: (i) =>
          moment()
  ...
  function createAddonWithVersionCompatibilities(emberVersions) {
    ...
    let emberVersionCompatibilities = emberVersions.map((emberVersion) => {
      ...
      if (typeof emberVersion === 'object') {
        ...
        compatible = Object.prototype.hasOwnProperty.call(
          emberVersion,
          'compatible'
        )
          ? emberVersion.compatible
          : true;
      }
      return server.create('emberVersionCompatibility', {
    ...
    testResult.update({
      emberVersionCompatibilityIds: emberVersionCompatibilities.map(
        (x) => x.id
      ),
      versionId: version.id,
    });
