...
module('Acceptance: Index', function (hooks) {
  ...
  test('visiting /', async function (assert) {
    ...
    assert
      .dom('.test-category')
      .exists({ count: 8 }, 'All categories should display');
    assert.ok(
      findByText('.test-category', 'Authentication (5)'),
      'Categories should list title and count of addons'
    );
    let firstSubcategory = findAll('.test-subcategory')[0];
    assert
      .dom(firstSubcategory)
      .containsText(
        'Simple Auth (1)',
        'Subcategories should display under category'
      );
    ...
    assert.equal(
      ...
      'URL should use category name token'
    );
    assert
      .dom('.test-category-header')
      .hasText('Authentication', 'Header should display');
    assert
      .dom('.test-category-description')
      .containsText('Addons for auth', 'Description should display');
    assert
      .dom('.test-addon-row')
      .exists({ count: 4 }, 'All addons in category should display');
    assert
      .dom('.test-addon-table-count')
      .hasText('Displaying 4 addons', 'Should show addon count');
    ...
    assert.equal(
      currentURL(),
      '/categories/simple-auth',
      'URL should use category name token'
    );
    assert
      .dom('.test-category-header')
      .containsText('Simple Auth', 'Header should display');
    assert
      .dom('.test-category-description')
      .containsText('Simple Auth addons', 'Description should display');
    assert
      .dom('.test-addon-row')
      .exists('All addons in category should display');
    assert
      .dom('.test-parent-category-link')
      .hasText('Authentication (5)', 'Should link to parent category');
    assert
      .dom('.test-addon-table-count')
      .hasText('Displaying 1 addon', 'Should show addon count');
  });
  ...
  test('including readme matches in search', async function (assert) {
    ...
    server.get('/search', (db, request) => {
      assert.equal(
        ...
        'test',
        'Query is sent to readme search'
      );
      return {
        search: [
          {
            addon_id: addon1.id,
            matches: ['testing stuff', 'more testing tips'],
    ...
    assert
      .dom('.addon-results .addon-list li')
      .containsText('ember-test-thing');
    ...
    assert
      .dom('.readme-results li')
      .doesNotExist('No readme results count showing');
  ...
  test('readme matches are sanitized', async function (assert) {
    ...
    server.get('/search', () => {
      return {
        search: [
          {
            addon_id: addon.id,
            matches: ['<style> a { color: "green"; }</style>'],
          }, // eslint-disable-line camelcase
        ],
      };
    });
    ...
    assert
      .dom('.test-readme-match style')
      .doesNotExist('<style> tag is stripped');
  });
  ...
  function testSearch(url, assertForContentOnUrl) {
    test(`visiting ${url} with a query`, async function (assert) {
      ...
      assert
        .dom('.addon-results .addon-list li')
        .exists({ count: 1 }, 'Only 1 addon result');
      assert.dom('.addon-results .addon-list li').containsText('ember-test-me');
      assert
        .dom('.category-results li')
        .exists({ count: 2 }, '2 matching categories');
      ...
      assert
        .dom('.addon-results .addon-list li')
        .exists({ count: 2 }, '2 addon results');
