...
module('Integration | Component | build-result-output', function (hooks) {
  ...
  module('JSON-format test output', function () {
    test('renders command groups', async function (assert) {
      this.set('buildResult', {
        output: JSON.stringify([
          {
            group: 'Group One',
            commands: [{ command: 'ls', output: 'foo' }],
          },
          {
            group: 'Group Two',
            commands: [
      ...
      assert
        .dom('section')
        .exists({ count: 2 }, 'renders <section> tags for each group');
    });
    ...
    test('displays command group details', async function (assert) {
      ...
      assert
        .dom('section > details > summary')
        .hasText('Group One', 'displays group header');
      assert
        .dom('section > details > details > summary')
        .hasText('ls', 'displays command in a <summary>');
      assert
        .dom('section > details > details > output')
        .hasText('foo', 'displays command output as an <output>');
    });
        ...
        output: JSON.stringify([
          {
            ...
            commands: [
              {
                command: 'ls foo',
                output: 'ls: foo: No such file or directory',
                failed: true,
              },
            ],
          },
