  ...
  */
  ...
  this.get('/categories', function (schema, request) {
    if (request.queryParams['filter[id]']) {
      let categories = schema.categories.find(
        request.queryParams['filter[id]'].split(',')
      );
  ...
  this.get('/addons', function (schema, request) {
    if (request.queryParams['filter[name]']) {
      let addonsIds = request.queryParams['filter[name]']
        .split(',')
        .map((name) => {
    ...
    if (request.queryParams['filter[inCategory]']) {
      let category = schema.categories.find(
        request.queryParams['filter[inCategory]']
      );
  ...
  this.get('/maintainers', function (schema, request) {
    if (request.queryParams['filter[name]']) {
      let maintainers = schema.maintainers.where({
        name: request.queryParams['filter[name]'],
      });
  ...
  let filterByRelatedAddonName = (schema, resourceTableName, request) => {
    if (request.queryParams['filter[addonName]']) {
      let addons = schema.addons.where({
        name: request.queryParams['filter[addonName]'],
      });
  ...
  this.patch('/addons/:id', function (schema, request) {
    ...
    let categories = schema.categories.find(
      JSON.parse(request.requestBody).data.relationships.categories.data.mapBy(
        'id'
      )
    );
