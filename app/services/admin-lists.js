...
export default class AdminListsService extends Service {
  ...
  addonsNeedingCategorization() {
    return this.store.query('addon', {
      filter: { isWip: false, notCategorized: true, hasRepoUrl: true },
  ...
  addonsNeedingReReview() {
    return this.store.query('addon', {
      filter: { needsReReview: true, hasRepoUrl: true },
      sort: '-latestVersionDate',
    });
  }
  ...
  addonsNeedingReview() {
    return this.store.query('addon', {
      filter: { notReviewed: true, isWip: false, hasRepoUrl: true },
      sort: '-latestVersionDate',
    });
  }
